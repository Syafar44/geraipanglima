    ...
    <>
      ...
      <section className="px-5 py-10 md:px-20 md:py-10 lg:px-32 lg:py-24">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
          {lanchBox.map((lanch) => {
            return (
              <div key={lanch.id}  className="hover:bg-red-700 hover:text-white rounded-b-xl transition-all duration-300 ease-in-out">
                <Link href={lanch.link}>
                  <img src={lanch.image} alt={lanch.link} className="w-full shadow-md" />
                  <p className="text-2xl font-semibold text-center py-3">{lanch.title}</p>
                </Link>
              </div>
            );
          })}
